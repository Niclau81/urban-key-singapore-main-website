...
import { Bath, BedDouble, Bookmark, Building2, MapPin, MoveUpRight, TrainFront } from "lucide-react";
...
export function PropertyCard({ property, featured = false }: { property: Property; featured?: boolean }) {
  ...
  const price = property.mode === "Rent" && property.monthlyRent ? `${money(property.monthlyRent)} / mo` : money(property.price);
  return (
    <article className={`group overflow-hidden rounded-[24px] border border-[#17382f]/10 bg-white shadow-[0_18px_60px_rgba(21,50,42,0.08)] transition duration-300 hover:-translate-y-1 hover:shadow-[0_26px_70px_rgba(21,50,42,0.14)] ${featured ? "lg:grid lg:grid-cols-[1.18fr_.82fr]" : ""}`}>
      <div className={`relative overflow-hidden ${featured ? "min-h-[350px]" : "aspect-[1.38/1]"}`}>
        ...
        <div className="absolute inset-x-0 top-0 flex items-start justify-between p-4">
          <span className="rounded-full bg-[#fbfaf7]/92 px-3 py-1.5 text-[10px] font-bold uppercase tracking-[0.15em] text-[#17382f] backdrop-blur">{property.mode === "Rent" ? "For rent" : "For sale"}</span>
      ...
      <div className={`flex flex-col p-5 ${featured ? "justify-center lg:p-8" : ""}`}>
        ...
        <div className="mb-5 flex flex-wrap items-center gap-x-4 gap-y-2 text-sm text-[#49635d]"><span className="flex items-center gap-1.5"><BedDouble className="size-4" />{property.beds}</span><span className="flex items-center gap-1.5"><Bath className="size-4" />{property.baths}</span><span className="flex items-center gap-1.5"><Building2 className="size-4" />{property.size.toLocaleString()} sq ft</span><span className="flex items-center gap-1.5"><TrainFront className="size-4 text-[#b68a4c]" />{property.mrtMinutes} min</span></div>
        <div className="mt-auto flex items-end justify-between border-t border-[#17382f]/10 pt-4"><div><p className="text-[10px] font-bold uppercase tracking-[0.13em] text-[#566c65]">Guide price</p><p className="mt-1 text-xl font-semibold text-[#17382f]">{price}</p></div><Link href={`/property/${property.id}`}><Button size="icon" className="rounded-full bg-[#17382f] text-white hover:bg-[#225245]" aria-label={`View ${property.title}`}><MoveUpRight className="size-4" /></Button></Link></div>
      </div>
    </article>
  );
}
