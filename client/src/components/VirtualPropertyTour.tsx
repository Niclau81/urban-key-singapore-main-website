  ...
  return <section ref={tourRef} tabIndex={0} onKeyDown={onViewerKeyDown} aria-label={`${title} immersive virtual property tour`} className={`relative overflow-hidden bg-[#17171e] text-white outline-none focus-visible:ring-4 focus-visible:ring-[#5aa8ff] ${isFullscreen ? "h-full w-full" : "rounded-[28px] border border-[#17382f]/10 shadow-[0_24px_90px_rgba(12,22,20,.2)]"}`} data-virtual-property-tour="immersive-viewer" data-tour-capture-mode={isVerified360 ? "verified-360" : "guided-photo-fallback"}>
    <div className="flex min-h-[clamp(480px,72dvh,680px)] flex-col xl:flex-row">
      <div className="relative min-h-[430px] flex-1 overflow-hidden xl:h-[clamp(480px,72dvh,680px)] xl:min-h-0">
        {interactivePanoramaUrl ? <div className="absolute inset-0"><EquirectangularPanorama src={interactivePanoramaUrl} alt={`${title} ${activeRoom.label}, ${isVerified360 ? "verified equirectangular panorama" : `${activeTiming.label.toLowerCase()} illustrative generated panorama preview`}`} hotspots={connectedRooms.map(room => ({ id: room.id, label: room.label, x: room.viewerPosition.x, y: room.viewerPosition.y, direction: room.direction }))} onSelectHotspot={selectRoom} /></div> : <img key={visibleImageSource} src={visibleImageSource} alt={`${title} ${activeRoom.label}, ${activeTiming.label.toLowerCase()} guided photo view`} className="absolute inset-0 h-full w-full object-cover motion-safe:animate-in motion-safe:fade-in motion-safe:duration-300" style={{ transform: `scale(${zoom})` }} data-tour-layer="timed-photograph" data-tour-photo-source={visibleImageSource} data-tour-zoom={zoom.toFixed(1)} />}
        ...
        {!isVerified360 && timingPhotos.length > 1 && <div data-tour-control="visible-photo-timing-choices" className="absolute bottom-20 left-4 z-40 rounded-xl border border-[#f4d59a]/55 bg-[#17171e]/95 p-2 shadow-[0_12px_30px_rgba(0,0,0,.42)] backdrop-blur sm:bottom-24 sm:left-6"><p className="px-2 pb-1 text-[10px] font-bold uppercase tracking-[.15em] text-[#f4d59a]">Photo timing · changes daylight and outlook</p><div className="grid grid-cols-3 gap-1">{timingPhotos.map(photo => <Button key={photo.id} type="button" onClick={() => selectTiming(photo.id)} aria-pressed={activeTiming.id === photo.id} className={`min-h-10 rounded-lg px-3 text-xs ${activeTiming.id === photo.id ? "bg-[#d5ae72] text-[#172b25] hover:bg-[#ebc98d]" : "border border-white/20 bg-white/8 text-white hover:bg-white/15"}`}>{photo.label}</Button>)}</div></div>}
