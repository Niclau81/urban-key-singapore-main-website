...
import { ChevronLeft, ChevronRight, CircleDotDashed, Footprints, Layers3, Maximize2, Minimize2, ShieldCheck, Sparkles, Volume2, VolumeX, X, ZoomIn, ZoomOut } from "lucide-react";
...
export function VirtualPropertyTour({ title, gallery, tour, onAppointmentIntent }: Props) {
  ...
  const activeTimingId = activeTiming?.id as "morning" | "noon" | "night" | undefined;
  const panoramaPreviewUrl = (activeTimingId ? activeRoom?.panoramaPreviewByTiming?.[activeTimingId] : undefined) ?? tour.panoramaPreviewUrls?.[activeRoom?.id ?? ""];
  ...
  const connectedRooms = activeRoom?.connections?.map(connection => {
    const room = tour.rooms.find(candidate => candidate.id === connection.roomId);
    return room ? { ...room, direction: connection.direction } : null;
  ...
  const selectTiming = (nextTimingId: string) => {
    ...
    const hasTimedPanorama = Boolean(activeRoom.panoramaPreviewByTiming?.[nextTimingId as "morning" | "noon" | "night"]);
    setGuidedPhotoRoomIds(current => {
      const next = new Set(current);
      if (hasTimedPanorama) next.delete(activeRoom.id);
      else next.add(activeRoom.id);
      return next;
    });
  ...
  return <section ref={tourRef} tabIndex={0} onKeyDown={onViewerKeyDown} aria-label={`${title} immersive virtual property tour`} className={`relative overflow-hidden bg-[#17171e] text-white outline-none focus-visible:ring-4 focus-visible:ring-[#5aa8ff] ${isFullscreen ? "h-full w-full" : "rounded-[28px] border border-[#17382f]/10 shadow-[0_24px_90px_rgba(12,22,20,.2)]"}`} data-virtual-property-tour="immersive-viewer" data-tour-capture-mode={isVerified360 ? "verified-360" : "guided-photo-fallback"}>
    <div className="flex min-h-[clamp(480px,72dvh,680px)] flex-col xl:flex-row">
      <div className="flex min-h-[430px] flex-1 flex-col overflow-hidden xl:h-[clamp(480px,72dvh,680px)] xl:min-h-0">
        {!isVerified360 && timingPhotos.length > 1 && <section data-tour-control="top-photo-timing-choices" className="shrink-0 border-b border-[#f4d59a]/30 bg-[#302d26] px-4 py-3 sm:px-6"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-[10px] font-bold uppercase tracking-[.15em] text-[#f4d59a]">{activeRoom.label} · photo timing</p><p className="mt-1 text-[11px] text-white/70">Illustrative daylight and exterior outlook treatment.</p></div><div className="grid w-full grid-cols-3 gap-1 sm:w-auto sm:min-w-[300px]">{timingPhotos.map(photo => <Button key={photo.id} type="button" onClick={() => selectTiming(photo.id)} aria-pressed={activeTiming.id === photo.id} className={`min-h-10 rounded-lg px-2 text-xs ${activeTiming.id === photo.id ? "bg-[#d5ae72] text-[#172b25] hover:bg-[#ebc98d]" : "border border-white/20 bg-white/8 text-white hover:bg-white/15"}`}>{photo.label}</Button>)}</div></div></section>}
      <div className="relative min-h-[430px] flex-1 overflow-hidden xl:min-h-0">
        {interactivePanoramaUrl ? <div className="absolute inset-0"><EquirectangularPanorama src={interactivePanoramaUrl} alt={`${title} ${activeRoom.label}, ${isVerified360 ? "verified equirectangular panorama" : `${activeTiming.label.toLowerCase()} illustrative generated panorama preview`}`} hotspots={connectedRooms.map(room => ({ id: room.id, label: room.label, x: room.viewerPosition.x, y: room.viewerPosition.y, direction: room.direction }))} onSelectHotspot={selectRoom} timeOfDay={activeTimingId} /></div> : <img key={visibleImageSource} src={visibleImageSource} alt={`${title} ${activeRoom.label}, ${activeTiming.label.toLowerCase()} guided photo view`} className="absolute inset-0 h-full w-full object-cover motion-safe:animate-in motion-safe:fade-in motion-safe:duration-300" style={{ transform: `scale(${zoom})` }} data-tour-layer="timed-photograph" data-tour-photo-source={visibleImageSource} data-tour-zoom={zoom.toFixed(1)} />}
        ...
        {!interactivePanoramaUrl && connectedRooms.map(room => <button key={room.id} type="button" onClick={() => selectRoom(room.id)} style={{ left: `${room.viewerPosition.x}%`, top: `${room.viewerPosition.y}%` }} aria-label={`Move ${room.direction} to ${room.label}`} className="absolute z-20 flex -translate-x-1/2 -translate-y-1/2 items-center gap-1 rounded-full border border-white/80 bg-[#087ff5] px-2 py-1.5 text-[10px] font-bold text-white shadow-[0_6px_20px_rgba(0,0,0,.45)] transition hover:scale-105 focus-visible:ring-4 focus-visible:ring-white/60 motion-reduce:transform-none motion-reduce:transition-none"><Footprints className="size-3" />{room.direction === "left" ? "←" : room.direction === "right" ? "→" : room.direction === "up" ? "↑" : "↓"} {room.label}</button>)}
        <div className="absolute inset-x-0 bottom-0 z-20 flex flex-col gap-3 p-4 sm:p-6 lg:flex-row lg:items-end lg:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2"><Button aria-label="Previous room" onClick={() => cycleRoom(-1)} size="icon" variant="ghost" className="size-10 rounded-full border border-white/20 bg-[#17171e]/80 text-white hover:bg-white/15"><ChevronLeft className="size-4" /></Button>{!isVerified360 && !isIllustrativePanoramaPreview && tour.captureMode === "illustrative-panorama" && panoramaPreviewUrl && <Button onClick={showPanoramaPreview} variant="outline" className="h-10 rounded-full border-white/20 bg-[#17171e]/85 px-4 text-xs text-white hover:bg-white/15">Return to panorama</Button>}<Button aria-label="Next room" onClick={() => cycleRoom(1)} size="icon" variant="ghost" className="size-10 rounded-full border border-white/20 bg-[#17171e]/80 text-white hover:bg-white/15"><ChevronRight className="size-4" /></Button></div>
        </div>
      </div>
      </div>
      ...
      <aside className="flex w-full shrink-0 flex-col border-t border-white/10 bg-[#24242c] xl:w-[340px] xl:border-l xl:border-t-0" aria-label="Tour floor and panorama navigator">
        ...
        <div className="max-h-[400px] space-y-4 overflow-y-auto p-4 xl:max-h-[calc(72dvh-250px)]">{tour.floors.map(floor => {
          ...
          return <section key={floor.id} className={`rounded-2xl border p-3 transition motion-reduce:transition-none ${floorActive ? "border-[#5aa8ff]/60 bg-white/5" : "border-white/10 bg-black/10"}`}><div className="flex items-center justify-between"><button type="button" onClick={() => selectFloor(floor.id)} className="text-sm font-semibold text-white hover:text-[#84c2ff]">{floor.label}</button><Layers3 className="size-4 text-white/45" /></div><div className="relative mt-3 aspect-[1.62] overflow-hidden rounded-xl border border-white/10 bg-[#f6f7f4]" aria-label={`${floor.label} floor view`}>{floorRooms.map(room => { const bounds = room.floorPlanBounds ?? { x: room.floorPlanPosition.x - 12, y: room.floorPlanPosition.y - 10, width: 24, height: 20 }; return <button key={room.id} type="button" onClick={() => selectRoom(room.id)} style={{ left: `${bounds.x}%`, top: `${bounds.y}%`, width: `${bounds.width}%`, height: `${bounds.height}%` }} aria-label={`Jump to ${room.label}`} aria-pressed={room.id === activeRoom.id} className={`absolute z-10 flex items-center justify-center rounded-md border-2 px-1 text-center text-[9px] font-semibold transition motion-reduce:transition-none ${room.id === activeRoom.id ? "border-[#d5ae72] bg-[#f7ebd7] text-[#172b25]" : "border-[#6d8880] bg-[#e5eee7] text-[#294139] hover:bg-white"}`}><span>{room.label}</span><span className={`absolute bottom-1 right-1 size-2 rounded-full border border-white ${room.id === activeRoom.id ? "bg-[#d5ae72]" : "bg-[#087ff5]"}`} /></button>; })}</div><div className="mt-3 grid gap-1">{floorRooms.map(room => <button key={room.id} type="button" onClick={() => selectRoom(room.id)} aria-pressed={room.id === activeRoom.id} className={`flex items-center gap-2 rounded-lg px-2 py-1.5 text-left text-xs transition motion-reduce:transition-none ${room.id === activeRoom.id ? "bg-[#087ff5]/20 text-white" : "text-white/65 hover:bg-white/8 hover:text-white"}`}><span className="size-2 rounded-full bg-[#087ff5]" />{room.label}</button>)}</div></section>;
        })}</div>
