...
export function VirtualPropertyTour({ title, gallery, tour, onAppointmentIntent }: Props) {
  ...
  const timingPhotos = useMemo(() => getTimingPhotos(gallery[activeRoom?.imageIndex ?? 0]), [activeRoom?.imageIndex, gallery]);
  const activeTiming = timingPhotos.find(photo => photo.id === timingId) ?? timingPhotos[0];
  const isVerified360 = tour.captureMode === "verified-360" && Boolean(tour.panoramaUrl);
  ...
  return <section ref={tourRef} tabIndex={0} onKeyDown={onViewerKeyDown} aria-label={`${title} immersive virtual property tour`} className={`relative overflow-hidden bg-[#17171e] text-white outline-none focus-visible:ring-4 focus-visible:ring-[#5aa8ff] ${isFullscreen ? "h-full w-full" : "rounded-[28px] border border-[#17382f]/10 shadow-[0_24px_90px_rgba(12,22,20,.2)]"}`} data-virtual-property-tour="immersive-viewer" data-tour-capture-mode={isVerified360 ? "verified-360" : "guided-photo-fallback"}>
    <div className="flex min-h-[clamp(480px,72dvh,680px)] flex-col xl:flex-row">
      <div className="relative min-h-[430px] flex-1 overflow-hidden xl:min-h-0">
        {isVerified360 ? <iframe title={`${title} verified 360 degree panorama`} src={tour.panoramaUrl} allow="fullscreen; gyroscope; accelerometer" allowFullScreen className="absolute inset-0 h-full w-full border-0" /> : <img key={activeTiming.src} src={activeTiming.src} alt={`${title} ${activeRoom.label}, ${activeTiming.label.toLowerCase()} guided photo view`} className="absolute inset-0 h-full w-full object-cover motion-safe:animate-in motion-safe:fade-in motion-safe:duration-300" style={{ transform: `scale(${zoom})` }} data-tour-layer="timed-photograph" data-tour-photo-source={activeTiming.src} data-tour-zoom={zoom.toFixed(1)} />}
        ...
        <div className="absolute inset-x-0 bottom-0 z-20 flex flex-col gap-3 p-4 sm:p-6 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-xl"><p className="font-display text-xl text-white">{activeRoom.note}</p><p className="mt-1 text-xs text-white/75">{isVerified360 ? "Interactive panoramic room capture" : `${activeTiming.label} · ${activeTiming.timeRange} · ${activeTiming.description}`}</p></div>
          <div className="flex flex-wrap items-center gap-2"><Button aria-label="Previous room" onClick={() => cycleRoom(-1)} size="icon" variant="ghost" className="size-10 rounded-full border border-white/20 bg-[#17171e]/80 text-white hover:bg-white/15"><ChevronLeft className="size-4" /></Button>{!isVerified360 && timingPhotos.length > 1 && <label className="flex h-10 items-center rounded-full border border-white/20 bg-[#17171e]/85 px-3 text-xs font-semibold text-white shadow-lg"><Eye className="mr-2 size-3.5 text-[#d5ae72]" /><span className="sr-only">Select photo timing</span><select value={activeTiming.id} onChange={event => setTimingId(event.target.value)} data-tour-control="photo-timing-select" aria-label="Select an available photo timing" className="min-w-[116px] appearance-none bg-transparent pr-1 text-xs outline-none"><option className="text-[#17382f]" value="morning">Morning</option>{timingPhotos.some(photo => photo.id === "noon") && <option className="text-[#17382f]" value="noon">Noon</option>}{timingPhotos.some(photo => photo.id === "midnight") && <option className="text-[#17382f]" value="midnight">Midnight</option>}</select></label>}<Button aria-label="Next room" onClick={() => cycleRoom(1)} size="icon" variant="ghost" className="size-10 rounded-full border border-white/20 bg-[#17171e]/80 text-white hover:bg-white/15"><ChevronRight className="size-4" /></Button></div>
        </div>
      </div>
      ...
      <aside className="flex w-full shrink-0 flex-col border-t border-white/10 bg-[#24242c] xl:w-[340px] xl:border-l xl:border-t-0" aria-label="Tour floor and panorama navigator">
        ...
        <div className="border-t border-white/10 p-4"><Button onClick={requestViewing} className="min-h-11 w-full rounded-full bg-[#d5ae72] text-[#16241f] hover:bg-[#ebc98d]">Request a viewing</Button>{tour.aiGuide.enabled && <div data-tour-guide-scope="approved-metadata" className="mt-3 rounded-xl border border-[#d5ae72]/25 bg-[#302d26] p-3"><Button variant="ghost" onClick={() => setGuideOpen(open => !open)} className="h-auto w-full justify-start p-0 text-left text-xs font-semibold text-[#f5d9a6] hover:bg-transparent hover:text-[#fff4de]"><Sparkles className="mr-1.5 size-3.5" />Optional AI Tour Guide</Button>{guideOpen && <div className="mt-3 border-t border-white/10 pt-3"><p className="text-[11px] leading-5 text-white/75">{tour.aiGuide.intro}</p><Button type="button" variant="outline" size="sm" onClick={toggleGuideNarration} className="mt-3 min-h-9 rounded-full border-white/20 bg-white/5 text-[11px] text-white hover:bg-white/10">{isSpeaking ? <><VolumeX className="mr-1.5 size-3.5" />Stop guide</> : <><Volume2 className="mr-1.5 size-3.5" />Listen to guide</>}</Button></div>}</div>}<div className="mt-3 flex gap-2 text-[10px] leading-4 text-white/55"><ShieldCheck className="mt-0.5 size-3.5 shrink-0 text-[#d5ae72]" /><p>{tour.captureMode === "guided-photo" ? "Guided photo fallback. It is not presented as a 360° panorama." : "Verified 360° media is subject to publication review."} Privacy review and manual approval are required before real publication.</p></div></div>
      </aside>
    </div>
