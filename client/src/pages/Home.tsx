  ...
  return (
    <div className="min-h-screen bg-[#f8f6f0] text-[#16362e]">
      ...
      <section className="relative min-h-[760px] overflow-hidden bg-[#10231e] text-white">
        ...
        <div className="container relative z-10 grid min-h-[684px] items-center py-16 lg:grid-cols-[1.05fr_.95fr]">
          <div className="max-w-[720px] pb-10 lg:pb-16">
            <div className="mb-7 flex items-center gap-3 text-[11px] font-bold uppercase tracking-[0.2em] text-[#d5ae72]"><span className="h-px w-10 bg-current" />{market.countryName} property, understood</div>
            ...
            <div className="mt-10 max-w-[760px] rounded-[22px] bg-[#faf9f5] p-2.5 text-[#16362e] shadow-[0_30px_90px_rgba(0,0,0,0.26)]">
              ...
              <div className="grid gap-2 pt-2 sm:grid-cols-[1.5fr_1fr_auto]">
                <div className="flex items-center gap-2 rounded-xl px-3"><Search className="size-4 text-[#9b7440]" /><Input value={query} onChange={event => setQuery(event.target.value)} className="border-0 bg-transparent px-0 shadow-none focus-visible:ring-0" placeholder={`Home, ${market.geography.regionLabel.toLowerCase()}, ${market.terminology.transit} or street`} onKeyDown={event => event.key === "Enter" && submit()} /></div>
                <Select value={district} onValueChange={setDistrict}><SelectTrigger aria-label={market.geography.regionLabel} className="h-11 border-0 bg-[#edf1ed] shadow-none"><SelectValue /></SelectTrigger><SelectContent>{[allRegionsLabel, ...market.geography.regions].map(region => <SelectItem key={region} value={region}>{region}</SelectItem>)}</SelectContent></Select>
            ...
            <div className="mt-7 flex flex-wrap gap-x-8 gap-y-3 text-xs text-white/60"><span className="flex items-center gap-2"><BadgeCheck className="size-4 text-[#d5ae72]" />Privacy-first insights</span><span className="flex items-center gap-2"><TrainFront className="size-4 text-[#d5ae72]" />{market.terminology.transit}-aware discovery</span><span className="flex items-center gap-2"><ShieldCheck className="size-4 text-[#d5ae72]" />Context with disclaimers</span></div>
          </div>
        </div>
        <div className="absolute bottom-8 right-8 z-20 hidden rounded-2xl border border-white/15 bg-white/10 p-4 backdrop-blur-xl lg:block"><p className="text-[10px] font-bold uppercase tracking-[0.18em] text-white/55">Active market</p><p className="mt-2 text-sm"><span className="text-[#d5ae72]">{market.countryCode}</span> · geography and map profile loaded</p></div>
      </section>
      ...
      <section className="border-b border-[#17382f]/10 bg-[#f8f6f0] py-8"><div className="container grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{[
        [`3D ${market.geography.regionLabel.toLowerCase()} map`, "Explore context, not just pins", Map], ["Immersive viewing", "Tour day and night", Box], ["Transaction lens", "Read the price journey", ChartNoAxesCombined], ["AI matching", "Explain what fits and why", Sparkles],
      ...
      <section className="container py-24">
        ...
        {properties.length ? <div className="grid gap-6 lg:grid-cols-3">{properties.map(property => <PropertyCard key={property.id} property={property} />)}</div> : <div className="rounded-[28px] border border-dashed border-[#17382f]/20 bg-white/50 px-6 py-14 text-center"><p className="font-display text-2xl">{market.countryName} is ready for a local catalog.</p><p className="mx-auto mt-3 max-w-xl text-sm leading-6 text-[#4e665f]">Its map, currency, regional taxonomy, and property terminology are configured. Connect local listing data to publish market-specific inventory.</p></div>}
      </section>

      <section className="overflow-hidden bg-[#e9eee9] py-24"><div className="container grid items-center gap-12 lg:grid-cols-[.92fr_1.08fr]"><div className="relative"><div className="overflow-hidden rounded-[32px]"><img src="/manus-storage/interlace-aerial_74c51dd9.jpg" alt="Architectural residence" className="aspect-[1.1/1] w-full object-cover" /></div><div className="absolute -bottom-6 -right-4 max-w-[230px] rounded-2xl bg-[#17382f] p-5 text-white shadow-2xl sm:right-8"><Building className="size-5 text-[#d5ae72]" /><p className="mt-3 font-display text-xl">See the whole address.</p><p className="mt-2 text-xs leading-5 text-white/75">Building form, local context, transit radius, and neighborhood boundaries in one view.</p></div></div><div className="lg:pl-10"><p className="eyebrow">Spatial intelligence</p><h2 className="section-title mt-3">{market.countryName}, layered<br />around your priorities.</h2><p className="mt-6 max-w-xl text-base leading-7 text-[#49615a]">Tilt into a 3D perspective, reveal nearby transport, compare {market.geography.regionLabel.toLowerCase()} boundaries, and move from neighborhood scale to unit-level detail without losing context.</p><div className="mt-8 grid gap-4 sm:grid-cols-2">{["3D building footprints", `${market.terminology.transit} travel-time radii`, `${market.geography.regionLabel} boundary overlays`, "Clustered property signals"].map(item => <div key={item} className="flex items-center gap-3 rounded-xl border border-[#17382f]/10 bg-white/50 px-4 py-3 text-sm font-semibold"><BadgeCheck className="size-4 text-[#76552f]" />{item}</div>)}</div><Link href="/map"><Button className="mt-9 rounded-full bg-[#17382f] px-6 text-white">Open map intelligence <ArrowRight className="ml-2 size-4" /></Button></Link></div></div></section>
