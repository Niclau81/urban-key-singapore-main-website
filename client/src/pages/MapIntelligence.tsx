...
export default function MapIntelligence() {
  ...
  return <div className="h-screen overflow-hidden bg-[#10231e] text-[#17382f]"><BrandHeader tone="dark" /><main className="relative h-[calc(100vh-76px)]">
    ...
    {selected && <aside id="map-property-preview" className={`absolute bottom-4 right-4 z-20 w-[calc(100%-2rem)] max-w-[512px] overflow-hidden rounded-[24px] border border-white/60 bg-[#faf9f5] shadow-2xl sm:max-w-[410px] ${mobilePanelsHidden ? "hidden sm:block" : ""}`}><div className="relative h-44"><img src={selected.image} alt={selected.title} className="h-full w-full object-cover" /><button aria-label="Close property preview" onClick={() => setSelected(null)} className="absolute right-3 top-3 grid size-8 place-items-center rounded-full bg-white/90"><X className="size-4" /></button></div><div className="p-5"><p className="text-[10px] font-bold uppercase tracking-[.15em] text-[#76552f]">{selected.type} · {selected.district}</p><h2 className="mt-2 text-lg font-semibold">{selected.title}</h2><div className="mt-3 flex flex-wrap gap-x-4 gap-y-2 text-xs text-[#4e665f]">{selected.isCommercial ? <><span>{selected.commercialUsage?.split(" · ")[0]}</span><span>{selected.floorLoading} kN/m²</span><span>{selected.ceilingHeight} m clear</span></> : <span>{selected.beds} beds</span>}<span>{selected.size.toLocaleString()} sq ft</span><span>{selected.mrtMinutes} min MRT</span></div><div className="mt-5 flex items-center justify-between"><p className="text-lg font-semibold">{(selected.mode === "Rent" || selected.mode === "Rent-Out") && selected.monthlyRent ? `S$${selected.monthlyRent.toLocaleString()} / mo` : `S$${(selected.price / 1000000).toFixed(2)}M`}</p><Link href={`/property/${selected.id}`}><Button className="rounded-full bg-[#17382f]">View property {selected.isCommercial ? <Warehouse className="ml-2 size-4" /> : <Building2 className="ml-2 size-4" />}</Button></Link></div></div></aside>}
    <button type="button" aria-controls="map-intelligence-controls map-property-preview" aria-pressed={!mobilePanelsHidden} onClick={() => setMobilePanelsHidden(hidden => !hidden)} className="absolute right-4 top-4 z-30 inline-flex min-h-11 items-center gap-2 rounded-full bg-[#17382f] px-4 text-xs font-semibold text-white shadow-xl sm:hidden">{mobilePanelsHidden ? <Eye className="size-4" /> : <EyeOff className="size-4" />}{mobilePanelsHidden ? "Show panels" : "Hide panels"}</button>
  </main></div>;
}
