...
import { Box, Building2, Layers3, LocateFixed, MapPinned, Route, TrainFront, X } from "lucide-react";
...
export default function MapIntelligence() {
  ...
  const setupMap = useCallback((map: google.maps.Map) => {
    ...
    const markers = properties.map(property => {
      ...
      markerEl.className = "map-property-pin";
      markerEl.setAttribute("aria-label", `View ${property.title}`);
      markerEl.innerHTML = `<span>S$${property.price >= 1000000 ? `${(property.price / 1000000).toFixed(2)}M` : property.price.toLocaleString()}</span>`;
      ...
      marker.addListener("click", () => { setSelected(property); map.panTo({ lat: property.latitude, lng: property.longitude }); });
      return marker;
    });
    ...
    clusterEl.innerHTML = `<strong>${properties.length}</strong><span>homes</span>`;
    const cluster = new google.maps.marker.AdvancedMarkerElement({ map: null, position: { lat: 1.289, lng: 103.829 }, content: clusterEl, title: `${properties.length} properties` });
    cluster.addListener("click", () => { map.setZoom(13); map.panTo({ lat: 1.289, lng: 103.829 }); });
  ...
  return <div className="h-screen overflow-hidden bg-[#10231e] text-[#17382f]"><BrandHeader tone="dark" /><main className="relative h-[calc(100vh-76px)]">
    <MapView initialCenter={{ lat: 1.2921, lng: 103.8347 }} initialZoom={12} onMapReady={setupMap} forceFailureForTesting={forceMapFallback} className="h-full w-full" />
    <div className="absolute left-4 top-4 z-10 w-[calc(100%-2rem)] max-w-[330px] rounded-[24px] border border-white/60 bg-[#faf9f5]/94 p-5 shadow-2xl backdrop-blur-xl"><div className="flex items-start justify-between"><div><p className="eyebrow">Map intelligence</p><h1 className="mt-2 font-display text-2xl">Explore by context.</h1></div><span className="grid size-9 place-items-center rounded-full bg-[#e8eee9] text-[#275649]"><MapPinned className="size-4" /></span></div><p className="mt-3 text-xs leading-5 text-[#4e665f]">Tilt, zoom, and reveal the layers that shape a property decision.</p><div className="mt-5 grid gap-2">{[
      ["threeD", "3D building perspective", Box], ["districts", "District boundaries", Layers3], ["mrt", "MRT walk-time radius", TrainFront],
    ].map(([key, label, Icon]) => <div key={key as string} className="flex items-center gap-3 rounded-xl bg-white px-3 py-3"><span className="grid size-8 place-items-center rounded-full bg-[#edf1ed] text-[#275649]"><Icon className="size-4" /></span><span className="flex-1 text-xs font-semibold">{label as string}</span><Switch aria-label={label as string} checked={layers[key as keyof LayerState]} onCheckedChange={() => toggle(key as keyof LayerState)} /></div>)}</div><div className="mt-4 flex gap-2"><Button size="sm" variant="outline" className="flex-1 rounded-full bg-white" onClick={() => mapRef.current?.setCenter({ lat: 1.2921, lng: 103.8347 })}><LocateFixed className="mr-2 size-3.5" />Recenter</Button><Button onClick={() => toast.info("Choose a workplace or school to configure commute-time routing.")} size="sm" variant="outline" className="flex-1 rounded-full bg-white"><Route className="mr-2 size-3.5" />Commute</Button></div></div>
    ...
    {selected && <aside className="absolute bottom-4 right-4 z-20 w-[calc(100%-2rem)] max-w-[380px] overflow-hidden rounded-[24px] border border-white/60 bg-[#faf9f5] shadow-2xl"><div className="relative h-44"><img src={selected.image} alt={selected.title} className="h-full w-full object-cover" /><button aria-label="Close property preview" onClick={() => setSelected(null)} className="absolute right-3 top-3 grid size-8 place-items-center rounded-full bg-white/90"><X className="size-4" /></button></div><div className="p-5"><p className="text-[10px] font-bold uppercase tracking-[.15em] text-[#76552f]">{selected.district}</p><h2 className="mt-2 text-lg font-semibold">{selected.title}</h2><div className="mt-3 flex gap-4 text-xs text-[#4e665f]"><span>{selected.beds} beds</span><span>{selected.size.toLocaleString()} sq ft</span><span>{selected.mrtMinutes} min MRT</span></div><div className="mt-5 flex items-center justify-between"><p className="text-lg font-semibold">S${(selected.price / 1000000).toFixed(2)}M</p><Link href={`/property/${selected.id}`}><Button className="rounded-full bg-[#17382f]">View property <Building2 className="ml-2 size-4" /></Button></Link></div></div></aside>}
  </main></div>;
}
