...
export default function MapIntelligence() {
  ...
  const setupMap = useCallback((map: google.maps.Map) => {
    ...
    const markers = properties.map(property => {
      ...
      const value = (property.mode === "Rent" || property.mode === "Rent-Out") && property.monthlyRent ? `${formatMarketCurrency(property.monthlyRent, market, { notation: "compact", maximumFractionDigits: 1 })}/mo` : formatMarketCurrency(property.price, market, { notation: "compact", maximumFractionDigits: 1 });
      markerEl.innerHTML = `<small>${property.isCommercial ? "C/I" : "HOME"}</small><span>${value}</span>`;
    ...
    clusterEl.setAttribute("aria-label", `Zoom into ${properties.length} ${market.terminology.propertyPlural}`);
  ...
  }, [layers.districts, layers.mrt, layers.threeD, market, properties]);
  useEffect(() => {
