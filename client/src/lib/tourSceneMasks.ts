export type TourExteriorMask = {
  ...
  opacity: number;
  label: string;
  composition: "window-aperture" | "full-scene";
};

const SOLID_MASK = "linear-gradient(#000, #000)";

function svgMask(polygons: string[], invert = false) {
  const shapes = polygons.map(points => `<polygon points='${points}' fill='black'/>`).join("");
  const svg = invert
    ? `<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 1000 1000' preserveAspectRatio='none'><defs><mask id='cut'><rect width='1000' height='1000' fill='white'/>${shapes}</mask></defs><rect width='1000' height='1000' fill='black' mask='url(#cut)'/></svg>`
    : `<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 1000 1000' preserveAspectRatio='none'><rect width='1000' height='1000' fill='transparent'/>${shapes}</svg>`;
